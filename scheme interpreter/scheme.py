...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        #if not type(operands.first) is int:
        operands = operands.map(lambda args: scheme_eval(args, env))
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        return self.env.make_child_frame(self.formals, args)# END PROBLEM 12
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        #print('ops', operands)
        #print('self', self)
        #operands = scheme_eval(operands, env, True)
        func =  scheme_eval(complete_eval(scheme_apply(self, operands, env)), env)
        return func
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        #env.bindings[target] = eval_all(expressions.second, env)
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    #env.bindings[expressions.first] = expressions.second
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(clause, 1, 2)
        #if not scheme_symbolp(clause.first):
        #    raise SchemeError('hello')
        names = Pair(clause.first, names)
        values = Pair(scheme_eval(clause.second.first, env), values)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            operator = scheme_eval(first, env)
            operator = complete_eval(operator)
            check_procedure(operator)
            #operands = rest.map(lambda ops: scheme_eval(ops, env))
